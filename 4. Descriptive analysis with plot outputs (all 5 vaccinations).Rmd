...
#names of columns
names<-c("Num","LA.Code","Practice_code","Denom","Uptake","Susceptibles","Quarter_number","Vaccine")
col_type<-c("numeric","character","character","numeric","numeric","numeric","numeric","character")

# read data for each vaccine
# GP6in1 <- read_csv("File path >>> GP 6in1 merge length ways.csv", col_names = names, skip = 1)
GPMMR1y2 <- read_csv("GP MMR1 merge length ways.csv",col_names = names, skip = 1)
...
# Rota <- read_csv("GP rota merge length ways.csv",col_names = names, skip = 1)
...
sumUptake<- subset(merged_data, Practice_code!="0") %>% 
  group_by(Quarter_number, Vaccine) %>% 
  summarise(uptake = (sum(Vacc_numerator, na.rm = TRUE) / sum(Denom, na.rm = TRUE)) * 100, Denom = sum(Denom, na.rm = TRUE), NumGP=n_distinct(Practice_code, na.rm=TRUE) )
